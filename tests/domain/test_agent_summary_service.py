...
from modules.agent.summary_service import evaluate_summary_readiness, generate_summary_pack, stream_generate_summary_pack
...
def test_generate_summary_pack_returns_new_schema(monkeypatch):
    ...
    assert len(result.summary_pack["secondary_conclusions"]) == 4
...
def test_stream_generate_summary_pack_emits_section_events(monkeypatch):
    ...
    async def fake_secondary(*_args, **_kwargs):
        return _valid_summary_pack()["secondary_conclusions"]
    ...
    monkeypatch.setattr("modules.agent.summary_service._generate_secondary_sections_with_llm", fake_secondary)
